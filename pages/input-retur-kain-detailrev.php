...
ini_set("error_reporting", 1);
	$qryCek=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE `id`='$_GET[id]'");
...
$sql = "SELECT max(no_retur) FROM tbl_detail_retur WHERE tgl_buat LIKE '$bln%'";
...
$qryb=mysqli_query($con,"SELECT no_retur FROM tbl_detail_retur WHERE id_nsp='$_GET[id]' AND tgl_buat LIKE '$today%' LIMIT 1");
...
$rb=mysqli_fetch_array($qryb);
	if($_POST['save']=="save"){	
		...
		$qry1=mysqli_query($con,"INSERT INTO tbl_detail_retur SET
		`id_nsp`='$_GET[id]',
		`no_retur`='$kdbon',
		`po`='$po',
		`no_order`='$order',
                ...
                <label for="warna" class="col-sm-2 control-label">Warna / Jenis Kain </label>
                  	<div class="col-sm-3">
                    	<select class="form-control select2" name="warna" required>
							...
							<?php $sqlw1=mysqli_query($con,"SELECT warna,nokk,jenis_kain FROM tbl_aftersales WHERE po='$rCek[po]' and no_order='$rCek[no_order]' ORDER BY warna");
							while ($rwarna=mysqli_fetch_array($sqlw1)){ ?>
							<option value="<?php echo $rwarna['warna'].";".$rwarna['nokk'].":".$rwarna['jenis_kain'];?>"><?php echo $rwarna['warna']." ; ".$rwarna['jenis_kain'];?></option>
							<?php } ?>
						</select>
                  	</div>
            </div>
			<div class="form-group">
                <label for="nokk_ncp" class="col-sm-2 control-label">No KK NCP</label>
                  	<div class="col-sm-2">
                    	<select class="form-control select2" name="nokk_ncp" required>
							<option value="">Pilih</option>
							<?php $sqlkkncp=mysqli_query($con,"SELECT nokk FROM tbl_aftersales WHERE po='$rCek[po]' and no_order='$rCek[no_order]' ORDER BY nokk");
							while ($rkkncp=mysqli_fetch_array($sqlkkncp)){ ?>
							<option value="<?php echo $rkkncp['nokk'];?>"><?php echo $rkkncp['nokk'];?></option>
							<?php } ?>
						</select>
                  	</div>
            </div>
			<div class="form-group">
				<label for="nokk_akj" class="col-sm-2 control-label">No KK AKJ</label>
					<div class="col-sm-2">
						<input name="nokk_akj" type="text" class="form-control" id="nokk_akj" value="" placeholder="No KK AKJ">
                  	</div>
			</div>
			<div class="form-group">
...
<div class="box-footer">
	<?php 
	$qrycek1=mysqli_query($con,"SELECT * FROM tbl_detail_retur WHERE no_order='$rCek[no_order]' AND po='$rCek[po]' AND id_nsp='$_GET[id]'");
  	...
  	<div class="col-xs-12">
		...
		<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form1">
			...
			<div class="box-body">
				...
				<tbody>
					<?php 
					$sql=mysqli_query($con," SELECT * FROM tbl_detail_retur WHERE id_nsp='$rCek[id]' AND po='$rCek[po]' AND no_order='$rCek[no_order]' ORDER BY tgl_buat ASC");
