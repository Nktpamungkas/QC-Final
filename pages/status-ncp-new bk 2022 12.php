...
where a.DocumentNo='$nokk' and (d.DepartmentName='KK Oke' or p.CounterDepartmentID='60') order by p.ID DESC";
...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      ...
      <table class="table table-bordered table-hover table-striped nowrap" id="example3" style="width:100%">
        ...
        <tbody>
          <?php
	$no=1;			
	if($_SESSION['dept']!="QC"){$where=" AND dept='$_SESSION[dept]' ";}else{ $where=" ";}
	if($Dept!=""){$where1=" AND dept='$Dept' ";}else{ $where1=" ";}
	if($NCP!=""){$where2=" AND no_ncp='$NCP' ";}else{ $where2=" ";}		
	...
	FROM tbl_ncp_qcf_new WHERE status='Belum OK' ".$where.$where1.$where2." ORDER BY id ASC");
		...
		if(trim(posisi($row1['nokk']))=="KK Oke"){			
		$qryUpdate=mysqli_query($con,"UPDATE tbl_ncp_qcf_new SET status='OK' WHERE nokk='$row1[nokk]'");			
		}
		else{	
		...
		$r1=mysqli_fetch_array($sql);
		 ...
		 ?>
          <tr bgcolor="<?php echo $bgcolor; ?>">
			...
			<td align="center"><font size="-1"><?php echo $row1['tgl_buat'];?></font><br><div class="btn-group"><a href="pages/cetak/cetak_ncp_new.php?id=<?php echo $row1['id'];?>" class="btn btn-xs btn-danger <?php if($_SESSION['dept']!="QC"){ echo "disabled";}?>" target="_blank"><i class="fa fa-print"></i></a><a href="pages/cetak/cetak_ncp_new_pdf.php?id=<?php echo $row1['id'];?>" class="btn btn-xs btn-info <?php if($_SESSION['dept']!="QC"){ echo "disabled";}?>" target="_blank"><i class="fa fa-file-pdf-o"></i></a></div></td>
			...
			<a href="#" class="posisi_kk" id="<?php echo $nokk1; ?>"><?php echo $nokk1; ?></a><br> <?php //echo posisi($row1['nokk']); ?></td>
            ...
            <td align="center"><font size="-1"><?php echo $row1['po'];?></font></td>
			      <td align="center"><font size="-1"><?php echo $row1['no_order'];?></font><br><a href="PenyelesaianNew-<?php echo $row1['id']; ?>" class="btn"><span class="label label-danger"><?php echo $row1['no_ncp_gabungan'];?></span></a><br>
					  <?php echo $row1['nokk']; ?>
