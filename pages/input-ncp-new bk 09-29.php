	 ...
	 <div class="col-md-5"> 
	  ...
	  <div class="form-group">
		  <label for="no_ncp" class="col-sm-3 control-label">No NCP</label>
		  	...
		  	<div class="col-sm-3">
			<input name="kksalinan" type="text" class="form-control" id="kksalinan" value="<?php if($cek>0){echo $rcek['nokk_salinan'];}?>" placeholder="Kartu Salinan" >			
		  </div>
		  <div class="col-sm-2">
			<input name="salinan" type="text" class="form-control" id="salinan" 
			value="<?php if($cek>0){echo $rcek['salinan'];}?>" placeholder="Salinan" >			
		  </div> 
		...
		<div class="form-group">
		  ...
		  <select class="form-control select2" name="ceksalinan" id="ceksalinan" onChange="rd1();"> 
					...
					<?php 
					$qCek=mysqli_query($con,"SELECT no_ncp,dept,revisi,salinan FROM tbl_ncp_qcf_new WHERE nokk='$_GET[nokk]' ORDER BY id ASC");
					while($dCek=mysqli_fetch_array($qCek)){ 
						$noncpL= $dCek['no_ncp']." ".$dCek['dept']."".$dCek['revisi']." ".$dCek['salinan'];
					?>
...
if($_POST['save']=="Simpan"){
	...
	if($_POST['kain_gerobak']=="ya"){
	  ...
	  $ncpgabung=$ncp." ".$_POST['dept']."".$rev1." ".$_POST['salinan'];
...
if($_POST['save']=="Ubah"){
	  ...
	  $rev1=$rck['revisi'];	  
	  $ncpgabung=$_POST['no_ncp']." ".$_POST['dept']."".$rev1." ".$_POST['salinan'];  	  
	  }else{
	  $sqlCk=mysqli_query($con,"SELECT revisi FROM tbl_ncp_qcf_new WHERE nokk='$nokk' and no_ncp='$NCPNO' and dept='$_POST[dept]' ORDER BY id DESC LIMIT 1");
	  $rck=mysqli_fetch_array($sqlCk);	
	  $rev1=$rck['revisi']+1; 	  
  ...
  if (result.value) {
      window.open('pages/cetak/cetak_ncp_new.php?nokk=$nokk&no_ncp=$NCPNO&dept=$_POST[dept]&revisi=$rck[revisi]','_blank');
