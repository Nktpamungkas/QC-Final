<script type="text/javascript">
    function proses_nokk(){
        ...
        if (nokk == 0) {
            window.location.href='InputLapJahitShading';
        }else{
            window.location.href='InputLapJahitShading&'+nokk;
        }
    }
    ...
    function proses_shift() {
        ...
        if (nokk == "") {
            ...
            window.location.href='InputLapJahitShading&'+nokk+'&'+shift;
        }
    }
</script>
<?php
...
{
	$ceksql=mysqli_query($con,"SELECT * FROM `tbl_lap_jahit_shading` WHERE `nokk`='$_GET[nokk]' and `shift`='$_POST[shift]' AND DATE_FORMAT(tgl_update, '%Y-%m-%d') = DATE_FORMAT(now(), '%Y-%m-%d') LIMIT 1");
	...
	if($sql1){
        ...
        echo "<script>swal({
            ...
            if (result.value) {
                window.location.href='InputLapJahitShading&$_POST[nokk]';
...
$msql=mysqli_query($con,"SELECT * FROM `tbl_lap_jahit_shading` WHERE `nokk`='$nokk' and `shift`='$_GET[shift]' AND DATE_FORMAT(tgl_update, '%Y-%m-%d') = DATE_FORMAT(now(), '%Y-%m-%d')");
$row=mysqli_fetch_array($msql);
$crow=mysqli_num_rows($msql);

//Data sudah disimpan di database mysqli
$msql1=mysqli_query($con,"SELECT * FROM `tbl_lap_jahit_shading` WHERE `nokk`='$nokk' and `shift`='$_GET[shift]' ");
$row1=mysqli_fetch_array($msql1);
$crow1=mysqli_num_rows($msql1);

//Data sudah disimpan di database mysqli
$qryfin=mysqli_query($con,"SELECT * FROM `tbl_lap_jahit_shading` WHERE `nokk`='$nokk' ORDER BY id DESC");
...
<div class="box box-info">
    ...
    <div class="box-footer">
            ...
            <?php } ?>
            <button type="button" class="btn btn-warning pull-left" name="lihat" value="lihat" onClick="window.location.href='LihatDataJahitShading'"><i class="fa fa-search"></i> Lihat Data</button> 	   
    </div>
</div>
</form>
