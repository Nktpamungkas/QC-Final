...
<div class="box box-primary collapsed-box">
  <div class="box-header with-border">
    <h3 class="box-title"> Filter Laporan NCP Cancel/Disposisi <span class="pull-right-container">
  ...
  <form method="post" enctype="multipart/form-data" name="form1" class="form-horizontal" id="form1">
	...
	<div class="form-group">	
        <div class="col-sm-6">
              <select class="form-control select2" name="kategori" id="kategori" style="width: 20%;" required>
                <option value="">Pilih</option>
  ...
  if($Candis=="1"){ $sts_candis=" AND (`status`='Cancel' OR `status`='Disposisi') "; }else{ $sts_candis=" ";}
	$qry1=mysqli_query($con,"SELECT * FROM tbl_ncp_qcf_new WHERE $Wdept $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts $sts_dis $sts_cancel $sts_candis ORDER BY id ASC");
	$qrySUM=mysqli_query($con,"SELECT COUNT(*) as Lot, SUM(rol) as Rol,SUM(berat) as Berat FROM tbl_ncp_qcf_new WHERE $Wdept $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts $sts_dis $sts_cancel $sts_candis");
...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      ...
      <?php if($Candis=="1"){?>
      <a href="pages/cetak/excel_disposisiqc_new.php?awal=<?php echo $_POST['awal']; ?>&akhir=<?php echo $_POST['akhir']; ?>" class="btn btn-danger pull-right">Cetak Disposisi/Cancel</a>
      <?php }?>
      <?php if($Cancel=="1"){?>
      <a href="pages/cetak/excel_cancelncp_new.php?awal=<?php echo $_POST['awal']; ?>&akhir=<?php echo $_POST['akhir']; ?>" class="btn btn-danger pull-right">Cetak Cancel</a>
      <?php }?>
      <?php if($Disposisi=="1"){?>
      <a href="pages/cetak/excel_disposisincp_new.php?awal=<?php echo $_POST['awal']; ?>&akhir=<?php echo $_POST['akhir']; ?>" class="btn btn-danger pull-right">Cetak Disposisi</a>
      <?php }?>
		<!--<?php if($Awal!="") { ?> 
