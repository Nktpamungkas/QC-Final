...
<div class="row">
  ...
  <div class="col-xs-5">	
    <div class="box box-success">
      <div class="box-header with-border">
        ...
        <?php if($Awal!="") { ?><br><b>Periode: <?php echo tanggal_indo($Awal)." - ".tanggal_indo($Akhir); }?> Ket: <?php if($Kategori=="ALL"){echo "ALL";}elseif($Kategori=="hitung"){echo "NCP dihitung";}elseif($Kategori=="tidakhitung"){echo "NCP tidak dihitung";}elseif($Kategori=="gerobak"){echo "diGerobak";} ?></b>
        <div class="box-tools pull-right">
          <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
        </div>
      </div>
	...
	if($Cancel !="1"){
		...
		$sts="  ";
            ...
            $totaldDis=0;
            $totaldllDis=0;
            $qryAll=mysqli_query($con,"SELECT COUNT(*) AS jml_all, SUM(berat) AS berat_all FROM tbl_ncp_qcf_new WHERE $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND (masalah_dominan!='' OR masalah_dominan!=NULL) AND NOT status='Cancel'");
            $rAll=mysqli_fetch_array($qryAll);
            $qryAllDis=mysqli_query($con,"SELECT COUNT(*) AS jml_all, SUM(berat) AS berat_all FROM tbl_ncp_qcf_new WHERE $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND (masalah_dominan!='' OR masalah_dominan!=NULL) AND `status`='Disposisi' AND NOT status='Cancel'");
            $rAllDis=mysqli_fetch_array($qryAllDis);
            $qrydef=mysqli_query($con,"SELECT SUM(berat) AS berat, ROUND(COUNT(masalah_dominan)/(SELECT COUNT(*) FROM tbl_ncp_qcf_new WHERE $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND NOT status='Cancel'
            ...
            while($rd=mysqli_fetch_array($qrydef)){
              $qrydefDis=mysqli_query($con,"SELECT SUM(berat) AS berat, ROUND(COUNT(masalah_dominan)/(SELECT COUNT(*) FROM tbl_ncp_qcf_new WHERE $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND `status`='Disposisi' AND masalah_dominan='$rd[masalah_dominan]' AND NOT status='Cancel'
              AND (masalah_dominan!='' OR masalah_dominan!=NULL))*100,1) AS persen,
              masalah_dominan
              FROM
              `tbl_ncp_qcf_new`
  ...
  <div class="col-xs-5">	
      ...
      <div class="box-body">
        <table class="table table-bordered table-striped" style="width: 100%;">
          ...
          <tbody>
            ...
            FROM
            `tbl_ncp_qcf_new`
            WHERE DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND (dept!='' OR dept!=NULL) AND NOT status='Cancel' AND ncp_hitung='ya' 
            GROUP BY dept
            ORDER BY berat DESC LIMIT 5");
	...
	$qry1=mysqli_query($con,"SELECT * FROM tbl_ncp_qcf_new WHERE $Wdept $WKategori DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $sts ORDER BY id ASC");
    ...
    <div class="box">
      <div class="box-header with-border">
		...
		<div class="pull-right">
		  <a href="./index1.php?p=lappenyelesaian&awal=<?php echo $Awal; ?>&akhir=<?php echo $Akhir; ?>" class="btn btn-primary " target="_blank" data-toggle="tooltip" data-html="true" title="Laporan Penyelesaian"><i class="fa fa-file"></i> Detail Penyelesaian</a>	 			
		  <a href="pages/cetak/cetak_harianncp_new.php?&awal=<?php echo $Awal; ?>&akhir=<?php echo $Akhir; ?>&dept=<?php echo $Dept; ?>&kategori=<?php echo $Kategori; ?>&cancel=<?php echo $Cancel; ?>" class="btn btn-danger " target="_blank" data-toggle="tooltip" data-html="true" title="Laporan NCP"><i class="fa fa-print"></i> Cetak</a>			
	...
	while($row1=mysqli_fetch_array($qry1)){
		 ?>
          <tr bgcolor="<?php echo $bgcolor; ?>">
            ...
            <td align="center"><a href="Penyelesaian-<?php echo $row1['id']; ?>" class="btn <?php if(strtoupper($_SESSION['usrid'])!="ARIF"){ echo "disabled";}?>"><span class="label label-danger"><?php echo $row1['no_ncp_gabungan'];?></span></a></td>
