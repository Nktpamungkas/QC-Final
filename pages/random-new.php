...
function tampil1(){
	...
	if(document.forms['form3']['jns_test1'].value=="HUMIDITY"){
		...
		$("#f28").css("display", "none");  // To hidefc27
...
$noitem=$_GET['no_item'];
$sqlCek=mysqli_query($con,"SELECT * FROM tbl_tq_nokk WHERE no_item='$noitem' LIMIT 1");
$cek=mysqli_num_rows($sqlCek);
$rcek=mysqli_fetch_array($sqlCek);
$sqlCekR=mysqli_query($con,"SELECT *,
	CONCAT_WS(' ',rfc_note,rph_note, rabr_note, rbas_note, rdry_note, rfla_note, rfwe_note, rfwi_note, rburs_note, rrepp_note, rwick_note, rabsor_note, rapper_note, rfiber_note, rpillb_note, rpillm_note, rpillr_note, rthick_note, rgrowth_note, rrecover_note, rstretch_note, rsns_note, rsnab_note, rsnam_note, rsnap_note, rwash_note, rwater_note, racid_note, ralkaline_note, rcrock_note, rphenolic_note, rcm_printing_note, rcm_dye_note, rlight_note, rlight_pers_note, rsaliva_note, rh_shrinkage_note, rfibre_note,rpilll_note,rsoil_note,rapperss_note,rbleeding_note,rchlorin_note,rdye_tf_note,rhumidity_note,rodour_note,rnedle_note) AS rnote_g FROM tbl_tq_randomtest WHERE no_item='$noitem' ORDER BY id DESC LIMIT 1");
$cekR=mysqli_num_rows($sqlCekR);
$rcekR=mysqli_fetch_array($sqlCekR);
$sqlTR=mysqli_query($con,"SELECT * FROM tbl_tq_temp_random WHERE no_item='$noitem' LIMIT 1");
$cekTR=mysqli_num_rows($sqlTR);
$rcekTR=mysqli_fetch_array($sqlTR);

$sqlTR2=mysqli_query($con,"SELECT * FROM tbl_tq_temp_random2 WHERE no_item='$noitem' LIMIT 1");
$cekTR2=mysqli_num_rows($sqlTR2);
$rcekTR2=mysqli_fetch_array($sqlTR2);

$sqlRW=mysqli_query($con,"SELECT * FROM tbl_tq_random_warna WHERE no_item='$noitem' ORDER BY id DESC LIMIT 1");
			...
			<div class="col-md-6">
				<div class="form-group">
					...
					<div class="col-sm-4">
						...
						value="<?php if($cek>0){echo $rcek['no_hanger'];}else {echo $_GET['no_hanger'];}?>" required>  
					</div>
					<div class="col-sm-4">
						...
						value="<?php echo $_GET['no_item'];?>" required>
					</div>	
				</div>
				<?php
				if($noitem!=""){
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<textarea name="jenis_kain" class="form-control" readonly row="3"><?php if($cek>0){echo $rcek['jenis_kain'];}?></textarea>
					</div>
				</div>
				<?php } ?>
			</div>
...
<?php
if($_POST['physical_save']=="save" and $cekR>0 and $cekTR>0 and $cekTR2>0 and $cekRW>0){
  ...
  if (result.value) {
	 ...
	 window.location.href='RandomNew-$noitem'; 
  }
});</script>";
	}
	}else if($_POST['physical_save']=="save" and $cekR>0 and $cekTR>0 and $cekTR2>0 and $cekRW==0){
		$sqlPHY=mysqli_query($con,"UPDATE tbl_tq_randomtest SET
			  `rflamability`='$_POST[rflamability]',
			  `rfla_note`='$_POST[rfla_note]',
			  `rfc_cott`='$_POST[rfc_cott]',
	  ...
	  if (result.value) {
		...
		$sqlPHY=mysqli_query($con,"UPDATE tbl_tq_randomtest SET
		  ...
		  `rss_washes15`='$_POST[rss_washes15]',
		  `rss_cmt`='$_POST[rss_cmt]',
		  `rshrinkage_l1`='$_POST[rshrinkage]',
		  `rshrinkage_l1`='$_POST[rshrinkage_len1]',
		  `rshrinkage_l2`='$_POST[rshrinkage_len2]',
		  `rshrinkage_l3`='$_POST[rshrinkage_len3]',
		  `rshrinkage_l4`='$_POST[rshrinkage_len4]',
...
}elseif($_POST['functional_save']=="save" and $cekR>0 and $cekTR>0 and $cekTR2>0 and $cekRW==0){
	$sqlFPH=mysqli_query($con,"UPDATE tbl_tq_randomtest SET
	`rwick_l1` = '$_POST[rwick_l1]',
	`rwick_w1` = '$_POST[rwick_w1]',
	`rwick_l2` = '$_POST[rwick_l2]',
