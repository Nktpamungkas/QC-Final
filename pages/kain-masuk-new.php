...
//$query = mysqli_query($sqlnotes) or die (mysqli_error());

//$testno = mysqli_fetch_array($query);
...
$sqlCek=mysqli_query($con,"SELECT a.*,b.hangtag FROM tbl_tq_nokk a LEFT JOIN tbl_master_hangtag b ON a.no_item = b.no_item WHERE nokk='$nokk' ORDER BY id DESC LIMIT 1");
	 ...
	 <div class="col-md-6">		 
		<div class="form-group">
					  ...
					  <input name="nokk" type="text" class="form-control" id="nokk" 
						 onchange="window.location='KainInNew-'+this.value" value="<?php echo $_GET['nokk'];?>" placeholder="No KK" required >
			  </div>
					</div>
        <div class="form-group">
		...
		<div class="form-group">
			<label for="style" class="col-sm-3 control-label">Style</label>
				<div class="col-sm-8">
					...
					value="<?php if($cek>0){echo $rcek['style'];}else if($_POST['style']!=""){echo $style;}else{ echo $r['OtherDesc'];} ?>" placeholder="Style">
				</div>				   
        </div> 
		<div class="form-group">
...
if($_POST['simpan_season']=="Simpan"){
  ...
  if (result.value) {
         window.location.href='KainInNew-$nokk';
