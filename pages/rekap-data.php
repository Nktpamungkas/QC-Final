...
$LotERP 	= isset($_POST['lot_erp_qcf']) ? $_POST['lot_erp_qcf'] : '';
$DemandERP 	= isset($_POST['demand']) ? $_POST['demand'] : '';
?>
...
<div class="box box-info">
  ...
  <form method="post" enctype="multipart/form-data" name="form1" class="form-horizontal" id="form1">
    ...
    <div class="form-group">
      <div class="col-sm-2">
        <input name="lot_erp_qcf" type="text" class="form-control" id="lot_erp_qcf" placeholder="Lot ERP" value="<?php echo $LotERP; ?>" />
      </div>
      <div class="col-sm-2">
        <input name="demand" type="text" class="form-control" id="demand" placeholder="Demand ERP" value="<?php echo $DemandERP; ?>" />
      </div>
    </div>
    <div class="form-group">
...
<thead class="bg-red">
   <tr>
      ...
      <th width="155" rowspan="2"><div align="center">Lot ERP</div></th>
      <th width="155" rowspan="2"><div align="center">Demand ERP</div></th>
...
<tbody>
  ...
  if($LotERP!=""){ $WLotERP=" AND lot_erp_qcf LIKE '$LotERP%' ";}else{ $WLotERP="";}
  if($DemandERP!=""){ $WDemandERP=" AND demand LIKE '$DemandERP%' ";}else{ $WDemandERP="";}	
  if($Order!=""){ $WOrder=" AND no_order LIKE '$Order%' ";}else{ $WOrder="";}
  if($PO!=""){ $WPO=" AND no_po LIKE '$PO%' ";}else{ $WPO="";}	
  if($Item!=""){ $WItem=" AND no_hanger LIKE '$Item%' ";}else{ $WItem="";}	
  if($Warna!=""){ $WWarna=" AND warna LIKE '$Warna%' ";}else{ $WWarna="";}
  if($Langganan!=""){ $WLangganan=" AND pelanggan LIKE '$Langganan%' ";}else{ $WLangganan="";}	
  	
  if($Awal=="" and $Order=="" and $Warna=="" and $Item=="" and $PO=="" and $Langganan=="" and $LotERP=="" and $DemandERP==""){
  	$sql=mysqli_query($con,"SELECT * FROM tbl_qcf
    WHERE no_order LIKE '$Order' AND no_po LIKE '$PO' AND no_hanger LIKE '$Item' AND warna LIKE '$Warna' AND pelanggan LIKE '$Langganan' AND lot_erp_qcf LIKE '$LotERP' AND demand LIKE '$DemandERP' $Where $Dly"); 
  }else{
	$sql=mysqli_query($con," SELECT * FROM tbl_qcf 
    WHERE NOT tgl_masuk = '' $WLangganan $WWarna $WItem $WPO $WOrder $WDemandERP $WLotERP $Where $Dly");
	}
	$col=0;
   ...
   <tr bgcolor="<?php echo $bgcolor; ?>">
     ...
     <td align="center"><?php echo $r['lot_erp_qcf'];?></td>
     <td align="center"><?php echo $r['demand'];?></td>
   ...
   <tr>
     ...
     <td align="right"><div align="center">Lot ERP</div></th>
     <td align="right"><div align="center">Demand ERP</div></th>
