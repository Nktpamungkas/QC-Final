...
<title>Cetak Disposisi QC</title>
<script>
...
<table width="100%">
  ...
  //$qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales $Where $shft $subdpt $disposisi $lgn ORDER BY pejabat ASC");
  if($Awal!="" or $GShift!="" or $Subdept!="" or $Disposisi=="1" or $Langganan!=""){
    $qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE langganan LIKE '%$_GET[langganan]%' $Where $disposisi $shft $subdpt ORDER BY pejabat,personil ASC");
    }else{
    $qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE langganan LIKE '$_GET[langganan]' $Where $disposisi $shft $subdpt ORDER BY pejabat,personil ASC");
    }
  $tkirim=0;
        ...
        <tr>
          <td style="border-top:0px #000000 solid; 
            ...
            <tr>
              <?php
              if($_GET['langganan']!=""){ $lgn =" AND langganan LIKE '%$_GET[langganan]%' "; }else{$lgn = " ";}
              $qry1=mysqli_query($con,"SELECT pejabat FROM tbl_aftersales WHERE
              DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' 
