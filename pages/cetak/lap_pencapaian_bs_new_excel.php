...
header("Content-Disposition: attachment; filename=lap_pencapaian_NCP_new_".substr($_GET['awal'],0,10)."_".substr($_GET['akhir'],0,10).".xls");//ganti nama sesuai keperluan
...
<table width="100%">
  <tr>
    ...
    date_format(tgl_buat,'%Y-%m-%d') as tgl_buat_pos FROM tbl_ncp_qcf_new WHERE date_format(tgl_rencana,'%Y-%m-%d') between '$Awal' and '$Akhir' $where1 ORDER BY date_format(tgl_rencana,'%Y-%m-%d') ASC");
