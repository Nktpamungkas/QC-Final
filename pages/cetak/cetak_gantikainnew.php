...
<table width="100%">
  ...
  FROM tbl_ganti_kain WHERE DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' GROUP BY langganan, id WITH ROLLUP");
  //$qrygk=mysqli_query("");
  $torder=0;
  $tkirim=0;
  $tfoc=0;
  ...
  $persensubtotal=0;
        ...
        $qryJML=mysqli_query($con,"SELECT SUM(x.qty_order) AS jml_qty_order, SUM(x.qty_kirim) AS jml_qty_kirim FROM
        (SELECT no_order, qty_order, qty_kirim FROM tbl_ganti_kain WHERE DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' GROUP BY no_order) X");
    ...
    <tr>
      ...
      <td align="center"><?php echo $TotalKirim;?></td>
    </tr>
    <tr>
      <td>PERSENTASE</td>
    ...
    <tr>
      ...
      <tr>
        ...
        $qryQC=mysqli_query($con,"SELECT SUM(qty_claim) AS qty_claim_qc FROM tbl_ganti_kain WHERE $Where AND (t_jawab='QCF' OR `t_jawab1`='QCF' OR `t_jawab2`='QCF')");
