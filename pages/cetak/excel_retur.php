...
<table width="100%" border="1">
    ...
    if($Awal!="" or $Order!="" or $PO!="" or $Langganan!=""){
        $query=mysqli_query($con,"SELECT * FROM tbl_detail_retur WHERE no_order LIKE '%$Order%' AND po LIKE '%$PO%' AND langganan LIKE '%$Langganan%' $Where $sts ORDER BY tgl_buat ASC ");
    }else{
        $query=mysqli_query($con,"SELECT * FROM tbl_detail_retur WHERE no_order LIKE '$Order' AND po LIKE '$PO' AND langganan LIKE '$Langganan' $Where $sts ORDER BY tgl_buat ASC");
    }
    $troll=0;
