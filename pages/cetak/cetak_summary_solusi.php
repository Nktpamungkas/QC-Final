  ...
  <tr>
    ...
    <tr>
        <td><table width="100%" border="1" class="table-list1">
            ...
            <tbody>
            <?php
            $qry=mysqli_query($con,"SELECT DISTINCT(solusi) FROM tbl_aftersales WHERE
            ...
            while($r=mysqli_fetch_array($qry)){
                $qryDisQC=mysqli_query($con,"SELECT COUNT(*) AS jml_dqc, SUM(qty_claim) as qty_claim_dqc FROM tbl_aftersales WHERE
                DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' 
                AND '$Akhir' AND sts_disposisiqc='1' AND solusi='$r[solusi]'");
                $rDisQC=mysqli_fetch_array($qryDisQC);
                $qryLolosQC=mysqli_query($con,"SELECT COUNT(*) AS jml_lqc, SUM(qty_claim) as qty_claim_lqc FROM tbl_aftersales WHERE
                DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' 
                AND '$Akhir' AND sts='1' AND solusi='$r[solusi]'");
                $rLolosQC=mysqli_fetch_array($qryLolosQC);
                $qryPro=mysqli_query($con,"SELECT COUNT(*) AS jml_pro, SUM(qty_claim) as qty_claim_pro FROM tbl_aftersales WHERE
                DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' 
