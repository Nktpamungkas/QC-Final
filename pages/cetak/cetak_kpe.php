...
<table width="100%">
  ...
  $qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE no_order LIKE '%$Order%' AND po LIKE '%$PO%' AND no_hanger LIKE '%$Hanger%' AND langganan LIKE '%$Langganan%' $Where ORDER BY masalah_dominan ASC");
  }else{
  $qry1=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE no_order LIKE '$Order' AND po LIKE '$PO' AND no_hanger LIKE '$Hanger' AND langganan LIKE '$Langganan' $Where ORDER BY masalah_dominan ASC");
  }
