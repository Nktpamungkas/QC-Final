...
<table width="100%" border="1">
	...
	<?php 
        ...
        (SELECT no_item, no_hanger, SUM(berat) AS qty_kg FROM tbl_ncp_qcf_new WHERE DATE_FORMAT(tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $wHitung
        GROUP BY no_item
        ORDER BY qty_kg DESC
        LIMIT 3) a");
        $rt3=mysqli_fetch_array($qryt3);
        $qrytall=mysqli_query($con,"SELECT SUM(berat) AS qty_kg_all FROM tbl_ncp_qcf_new WHERE DATE_FORMAT(tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' $wHitung ");
