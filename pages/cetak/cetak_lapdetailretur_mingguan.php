...
<table width="100%">
  ...
  FROM tbl_detail_retur WHERE DATE_FORMAT( tgl_buat, '%Y-%m-%d' ) BETWEEN '$Awal' AND '$Akhir' AND tgl_sjretur!='0000-00-00' ORDER BY tgltrm_sjretur ASC");
  //$qrygk=mysqli_query("");
  $troll=0;
  $tqty=0;
  $tqty_tu=0;
            while($row1=mysqli_fetch_array($qry1)){
        $qry2=mysqli_query($con,"SELECT t_jawab, t_jawab1, t_jawab2 FROM tbl_aftersales WHERE nokk='$row1[nokk]'");
    ...
    <tr>
        <td><table width="35%" border="1" class="table-list1">
            ...
            $qryQC=mysqli_query($con,"SELECT SUM(qty_tu) AS qty_claim_qc FROM tbl_detail_retur WHERE $Where AND (`t_jawab`='QCF' OR `t_jawab1`='QCF' OR `t_jawab2`='QCF')");
