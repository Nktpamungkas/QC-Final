  ...
  <body>
    <div class="box box-info">
      <div class="box-header with-border">
        <h3 class="box-title">Filter Grafik Per Periode <span class="pull-right-container">
...
LEFT JOIN(select dept,count(no_ncp) as jml from tbl_ncp_qcf_new where date_format(tgl_buat,'%Y-%m-%d') between '$Awal' AND '$Akhir'  group by dept 
) b ON a.nama=b.dept");
	$r=mysqli_fetch_array($qry);
	$qry1=mysqli_query($con,"SELECT GROUP_CONCAT('''',a.masalah_dominan,'''') masalah,GROUP_CONCAT(IFNULL(b.jml,0)) as jml FROM 
(select * from (
select masalah_dominan,count(masalah_dominan) as jml  from tbl_ncp_qcf_new where date_format(tgl_buat,'%Y-%m-%d') between '$Awal' AND '$Akhir'  group by masalah_dominan) a
where jml>10) a
