...
$sqlCek=mysqli_query($con,"SELECT * FROM tbl_ncp_qcf_testing WHERE nokk='$nokk' and no_ncp_gabungan='$ncpG' ORDER BY id DESC LIMIT 1");
$cek=mysqli_num_rows($sqlCek);
$rcek=mysqli_fetch_array($sqlCek);
$sqlCek0=mysqli_query($con,"SELECT salin_urut FROM tbl_ncp_qcf_testing WHERE nokk_salinan='$kksalin'  ORDER BY id DESC LIMIT 1");
 ...
 <div class="box box-info">
   <div class="box-header with-border">
    <h3 class="box-title">Input Data Kartu Kerja Untuk NCP &nbsp;<span class="pull-right-container">
              <small class="label pull-right bg-green blink_me">new1</small>
            </span></h3>
	 ...
	 <div class="col-md-5"> 
		<div class="form-group">
					  ...
					  <input name="nokk" type="text" class="form-control" id="nokk" 
						 onchange="window.location='NCPTest-'+this.value" value="<?php echo $_GET['nokk'];?>" placeholder="No KK" required >
			  </div>
			<?php if($nokk){ ?>
	  ...
	  <div class="form-group">
		  <label for="no_ncp" class="col-sm-3 control-label">Kk Salinan</label>
		  	<div class="col-sm-2">
			<input name="kksalinan" type="text" class="form-control" id="kksalinan" 
				   onchange="window.location='NCPTest-<?php echo $nokk;?>-'+this.value"
				   value="<?php if($cek>0){echo $rcek['nokk_salinan'];}else{ echo $kksalin; }?>" placeholder="Kartu Salinan" >			
		  </div>
		  <div class="col-sm-2">
			<input name="salinan" type="text" class="form-control" id="salinan" 
			value="<?php if($cek>0){echo $rcek['salinan'];}else{ if($Ksalin!=""){echo $Ksalin."-".$urutS;} }?>" placeholder="Salinan Urutan" readonly>			
		  </div> 
		  <div class="col-sm-2">
			<input name="salin" type="text" class="form-control" id="salin" 
			value="<?php if($cek>0){echo $rcek['salin'];}?>" placeholder="Salin Induk" >			
		  </div>
		</div>	  
	  <div class="form-group">
		  <label for="no_ncp" class="col-sm-3 control-label">No NCP</label>
		  	<div class="col-sm-3">
				...
				<select name="no_ncp" class="form-control" id="no_ncp" placeholder="No NCP" onChange="rd();">
					<option value=""></option>
					<?php $qCek=mysqli_query($con,"SELECT no_ncp,masalah FROM tbl_ncp_qcf_testing WHERE nokk='$_GET[nokk]' GROUP BY no_ncp");
					while($dCek=mysqli_fetch_array($qCek)){ ?>
					<option value="<?php echo $dCek['no_ncp'];?>" <?php if($dCek['no_ncp']==$NCPNO or $rcek['no_ncp']==$dCek['no_ncp']){echo "SELECTED";}?>><?php echo $dCek['no_ncp'];?></option>
					<?php } ?>
				</select>
		...
		<div class="form-group">
		  ...
		  <select class="form-control select2" name="ceksalinan" id="ceksalinan" onChange="rd1();"> 
					...
					<?php 
					$qCek=mysqli_query($con,"SELECT no_ncp,dept,revisi,salinan,salin,no_ncp_gabungan FROM tbl_ncp_qcf_testing WHERE nokk='$_GET[nokk]' ORDER BY id ASC");
					while($dCek=mysqli_fetch_array($qCek)){ 
						//$noncpL= $dCek['no_ncp']." ".$dCek['dept']."".$dCek['revisi']." ".$dCek['salinan']."/".$dCek['salin'];
						$noncpL=$dCek['no_ncp_gabungan'];
					?>
					<option value="<?php echo $noncpL;?>" <?php if($noncpL==$CekSalinan){echo "SELECTED";}?>><?php echo $noncpL;?></option>
					<?php } ?>
				</select>
		...
		<div class="form-group">
			...
			<label for="ncp_hitung" ><input type="checkbox" name="ncp_hitung" id="ncp_hitung" value="ya" <?php if($rcek['ncp_hitung']=="ya"){echo "CHECKED";}?>> &nbsp;
        ...
        <div class="col-sm-2">  
					<input name="rol" type="text" class="form-control" id="rol" value="<?php if($cek>0){echo $rcek['rol'];}else{if($r['RollCount']!="" AND ((strpos($lotno,'K') !== true) OR (strpos($rcek['lot'],'K') !== true))){echo round($r['RollCount']);}} ?>" placeholder="0.00" style="text-align: right;" required>			
        </div>
	    </div>   
		<div class="form-group">
          ...
          <div class="col-sm-3">
                    <div class="input-group">  
					<input name="berat" type="text" class="form-control" id="berat" value="<?php if($cek>0){echo $rcek['berat'];}else{if($r['Weight']!=""AND ((strpos($lotno,'K') !== true) OR (strpos($rcek['lot'],'K') !== true))){echo round($r['Weight'],2);}} ?>" placeholder="0.00" style="text-align: right;" required>
					<span class="input-group-addon">KGs</span>	
					</div>
                  </div>
			<label for="kain_gerobak" ><input type="checkbox" name="kain_gerobak" id="ncp_hitung" value="ya" <?php if($rcek['kain_gerobak']=="ya"){echo "CHECKED";}?>> &nbsp;
		...
		<div class="form-group">
		  ...
		  <input type="hidden" value="<?php if($cek>0){ echo $rcek['revisi']; } ?>" name="rev0">
		  <input type="hidden" value="<?php echo $urutS;//if($cek0>0){ echo $rcek0['salin_urut']; } ?>" name="salin_urut">
		  <input type="hidden" value="<?php if($cek>0){ echo $rcek['id']; } ?>" name="idncp">
	 </div>
...
$format = date("y/m/");
$sql=mysqli_query($con,"SELECT no_ncp FROM tbl_ncp_qcf_testing WHERE substr(no_ncp,1,6) like '".$format."%' ORDER BY round(substr(no_ncp,7,4)) DESC LIMIT 1 ") or die (mysqli_error());
...
if($_POST['save']=="Simpan"){
	...
	if($_POST['kain_gerobak']=="ya"){
	  ...
	  $ncp=$nou; }
	  $sqlCk=mysqli_query($con,"SELECT revisi FROM tbl_ncp_qcf_testing WHERE nokk='$nokk' and no_ncp='$NCPNO' and dept='$_POST[dept]' ORDER BY id DESC LIMIT 1");
	  ...
	  if(isset($_POST["rmp_benang"]))  
  	  ...
  	  $sqlData=mysqli_query($con,"INSERT INTO tbl_ncp_qcf_testing SET 
		  nokk='$_POST[nokk]',
		  no_ncp='$ncp',
  ...
  if (result.value) {
      window.open('pages/cetak/cetak_ncp_testing.php?nokk=$nokk&no_ncp=$ncp&dept=$_POST[dept]&revisi=$_POST[rev0]','_blank');
...
if($_POST['save']=="Ubah"){
	  $sqlCk=mysqli_query($con,"SELECT revisi,dept FROM tbl_ncp_qcf_testing WHERE id='$_POST[idncp]' ORDER BY id DESC LIMIT 1");
	  $rck=mysqli_fetch_array($sqlCk);	
      $ket=str_replace("'","''",$_POST['ket']);
	  $po=str_replace("'","''",$_POST['no_po']);
	  $lot=trim($_POST['lot']);
	...
	if($_POST['kain_gerobak']=="ya"){
	  ...
	  if(isset($_POST["rmp_benang"]))  
  	  ...
  	  $sqlData=mysqli_query($con,"UPDATE tbl_ncp_qcf_testing SET 
		  rol='$_POST[rol]',
		  masalah='$kt1',
		  berat='$_POST[berat]',
		  nsp='$_POST[nsp1]',
  ...
  if (result.value) {
      window.open('pages/cetak/cetak_ncp_testing.php?id=$_POST[idncp]','_blank');
