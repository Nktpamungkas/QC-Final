...
$format = $fk.date("y/m/");
$sql=mysqli_query($con,"SELECT no_bon FROM tbl_ganti_kain WHERE substr(no_bon,1,10) like '".$format."%' ORDER BY no_bon DESC LIMIT 1 ") or die (mysqli_error());
...
$today = date("Y-m-d");
$sql = "SELECT max(kd_ganti) AS kd_ganti FROM tbl_ganti_kain WHERE no_order='$rCek[no_order]'";
...
$qryg=mysqli_query($con,"SELECT kd_ganti FROM tbl_ganti_kain WHERE no_order='$rCek[no_order]' AND alasan='$_POST[alasan]' AND tgl_buat LIKE '$today%' LIMIT 1");
$cekg=mysqli_num_rows($qryg);
$rg=mysqli_fetch_array($qryg);
$qryb=mysqli_query($con,"SELECT no_bon FROM tbl_ganti_kain WHERE id_nsp='$_GET[id]' AND alasan='$_POST[alasan]' AND tgl_buat LIKE '$today%' LIMIT 1");
$cekb=mysqli_num_rows($qryb);
$rb=mysqli_fetch_array($qryb);
$qryd1=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE warna='$WN1' and lot='$L1' and no_order='$rCek[no_order]' and no_hanger='$rCek[no_hanger]' AND po='$rCek[po]' ORDER BY id DESC LIMIT 1");
$rowd1=mysqli_fetch_array($qryd1);
$qryr1=mysqli_query($con,"SELECT * FROM tbl_aftersales WHERE id='$_GET[id]'");
$r1=mysqli_fetch_array($qryr1);
	...
	if(isset($_POST['save'])){
		...
		$qry1=mysqli_query($con,"INSERT INTO tbl_ganti_kain SET
		`id_nsp`='$_GET[id]',
		`buyer`='$buyer',
		`kd_ganti`='$kdganti',
		`no_bon`='$kdbon',
                ...
                <label for="warna1" class="col-sm-2 control-label">Warna / Lot </label>
                  	<div class="col-sm-3">
                    	<select class="form-control select2" name="warna1" required>
							<option value="">Pilih</option>
							<?php $sqlw1=mysqli_query($con,"SELECT warna,lot FROM tbl_aftersales WHERE no_order='$rCek[no_order]' and no_hanger='$rCek[no_hanger]' and po='$rCek[po]' ORDER BY warna");
					...
					<label for="warna1" class="col-sm-2 control-label">Qty Setelah Negosiasi </label>
				  	<div class="col-sm-2">
                    	<div class="input-group">
							<?php $qryw1=mysqli_query($con,"SELECT qty_claim,nokk FROM tbl_aftersales WHERE warna='$WN1' and lot='$L1' and no_order='$rCek[no_order]' and no_hanger='$rCek[no_hanger]' and po='$rCek[po]'");
  	...
  	<div class="col-xs-12">
        ...
        <form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form2">
			...
			<div class="box-body">		
				...
				<tbody>
					<?php 
					$sql=mysqli_query($con," SELECT * FROM tbl_ganti_kain WHERE id_nsp='$_GET[id]' ORDER BY tgl_buat ASC");
